...
with open(dir_report + "/report.md", "w") as f:
    ...
    # The tested revision of this repository used to be part of the report
    # path. A link is both more useful and more discoverable than a bare
    # abbreviated hash buried in a directory name.
    hash = report.get("hash")
    if hash:
        f.write(
            "*Distribution revision:* [%s](%s)\n\n"
            % (hash[:8], os.path.join(repo, "commit", hash))
        )
